...
namespace Fylt.Api.Controllers
{
    ...
    public class RankingItemsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetItemsByRankingId([FromRoute] int rankingId)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la récupération des items pour le classement ID {RankingId}. Détail: {Message}", rankingId, ex.Message);
                return StatusCode(500, ApiResponseBase.Fail("Error interno al obtener los items del ranking.", 500));
            }
        }
        ...
        public async Task<IActionResult> Create([FromRoute] int rankingId, [FromBody] RankingItemVO newItem)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la création d'un item pour le classement ID {RankingId}. Détail: {Message}", rankingId, ex.Message);
                return StatusCode(500, ApiResponseBase.Fail("Error interno al crear el item del ranking.", 500));
            }
        }
        ...
        public async Task<IActionResult> Update([FromRoute] int itemId, [FromBody] RankingItemVO item)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la mise à jour de l'item ID {ItemId}. Détail: {Message}", itemId, ex.Message);
                return StatusCode(500, ApiResponseBase.Fail("Error interno al actualizar el item del ranking.", 500));
            }
        }
        ...
        public async Task<IActionResult> Delete([FromRoute] int itemId)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la suppression de l'item ID {ItemId}. Détail: {Message}", itemId, ex.Message);
                return StatusCode(500, ApiResponseBase.Fail("Error interno al eliminar el item del ranking.", 500));
            }
        }
