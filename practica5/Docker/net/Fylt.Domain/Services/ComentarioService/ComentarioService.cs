...
namespace Fylt.Domain.Services.ComentarioService
{
    public class ComentarioService : IComentarioService
    {
        ...
        public async Task<List<ComentarioVO>> GetAll()
        {
            ...
            var lista = ComentarioMappers.ToVOList(comentarios).ToList();
            foreach (var c in lista)
            {
                var user = _context.Usuarios.FirstOrDefault(x => x.Id == c.IdUser);
                c.Username = user.Username;
                c.ImagenPelicula = _context.Peliculas.FirstOrDefault(x => x.Id == c.IdPelicula).Imagen;
                c.FotoPerfil = user.Foto;
            }
            return lista;
        }
        ...
        public async Task<ComentarioVO?> GetById(int id)
        {
            ...
            return new ComentarioVO
            {
                ...
                ImagenPelicula= _context.Peliculas.FirstOrDefault(x => x.Id == c.IdPelicula).Imagen,
