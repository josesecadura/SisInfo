...
namespace Fylt.Api.Controllers
{
    ...
    public class ComentarioController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] ComentarioVO comentario, CancellationToken ct)
        {
            ...
            // ✅ Rellenar los campos que falten
            var anterior = await _comentarioService.GetById(id);
            if (anterior == null)
                return NotFound(ApiResponseBase.NotFound($"No se encontró el comentario con ID {id}."));

            if (comentario.Descripcion == null)
                comentario.Descripcion = anterior.Descripcion;
            if (comentario.IdUser == 0)
                comentario.IdUser = anterior.IdUser;
            if (comentario.IdPelicula == 0)
                comentario.IdPelicula = anterior.IdPelicula;
            if (comentario.NumLikes == 0)
                comentario.NumLikes = anterior.NumLikes;
